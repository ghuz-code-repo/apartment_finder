...
class DebtReminderSubscription(db.Model):
    ...
    Получатель адресуется логином портала (он же ключ): адрес доставки находит
    notification-service через auth-service, ник и chat_id здесь не хранятся.

    manager_id — снимок связки «логин ↔ менеджер CRM» на момент настройки.
    Связку админ потом меняет, поэтому перед каждой отправкой она
    пересчитывается (debt_reminder_service.refresh_manager_link): иначе
    напоминания продолжают уходить прежнему логину с чужой дебиторкой.
    """
    __bind_key__ = 'planning_db'
